...
class SetUI {
  private:
    ...
    // Takes a string as an input, then return the same string in a different
    // color.
    string ColorText(string s, COLOR color);

    // helper function that takes an input string and split it into parts (e.g.,
    // "5->3" becomes '5' and '3'). If this relation is a valid binary relation,
    // then add this relation to the 'relation' set. Return true if nothing went
  ...
  public:
    ...
    // all the error messages are predefined in this function so that they can
    // be called by controller whenever an error is detected.
    ...
    // Similar to other properties: reflexive, symmetric, transtivie, and
    ...
    // the printout should look like this: e.g., [0] = {1,2,3,4} where 1,2,3,4
    // are all having equivalence relation with 0. Any error message should be
    // handled in 'printError' function. NOT here.
    ...
    void printReachable(bool reachable, bool self);
