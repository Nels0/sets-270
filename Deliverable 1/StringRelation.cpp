...
void StringRelation::printRelation() {
    ...
    while (i < j) {
        ...
        if (i != j - 1) {
            cout << S->ColourText("(", RED) << S->ColourText(next, RED)
                 << S->ColourText(")", RED) << S->ColourText(", ", RED);
            // cout<<"("<<next<<")"<<", ";
            ++i;
        } else {
            cout << S->ColourText("(", RED) << S->ColourText(next, RED)
...
void StringRelation::printRelation(SetOfStrings *s) {
    ...
    if (!out->isEqual(s)) {
        cout
            << S->ColourText(
                   "Invalid argument -- argument must be a product set of set1",
                   RED)
            << "\n";
        return;
    } else {
        cout << "The elements of the relation that is equal to the product set "
                "are:"
             << S->ColourText("{", BLUE);
        int i = 0, j = 0;
        j = s->size();
        while (i < j) {
            ...
            if (i != j - 1) {
                cout << S->ColourText("(", BLUE) << S->ColourText(next, BLUE)
                     << S->ColourText(")", BLUE) << S->ColourText(", ", BLUE);
                ++i;
            } else {
                cout << S->ColourText("(", BLUE) << S->ColourText(next, BLUE)
                     << S->ColourText(")", BLUE);
